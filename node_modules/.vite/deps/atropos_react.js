...
//#region node_modules/atropos/atropos.mjs
...
function Atropos$1(originalParams) {
	if (originalParams === void 0) originalParams = {};
	var _originalParams = originalParams, el = _originalParams.el, eventsEl = _originalParams.eventsEl;
	var isComponent = originalParams.isComponent;
	var childrenRootEl;
	var self = {
		__atropos__: true,
		params: _extends$1({}, defaults, {
			onEnter: null,
			onLeave: null,
			onRotate: null
	...
	var createShadow = function createShadow() {
		...
		if (created) rotateEl.appendChild(shadowEl);
	};
	var createHighlight = function createHighlight() {
		...
		if (created) innerEl.appendChild(highlightEl);
	};
	var setChildrenOffset = function setChildrenOffset(_ref) {
		...
		var getOpacity = function getOpacity(element) {
			if (element.dataset.atroposOpacity && typeof element.dataset.atroposOpacity === "string") return element.dataset.atroposOpacity.split(";").map(function(v) {
				return parseFloat(v);
			});
		};
		$$(childrenRootEl, "[data-atropos-offset], [data-atropos-opacity]").forEach(function(childEl) {
			...
			if (rotateXPercentage === 0 && rotateYPercentage === 0) {
				...
				if (!Number.isNaN(childElOffset) && !opacityOnly) $setTransform(childEl, "translate3d(" + -rotateYPercentage * -childElOffset + "%, " + rotateXPercentage * -childElOffset + "%, 0)");
				if (elementOpacity) {
	...
	var setElements = function setElements(clientX, clientY) {
		...
		if (!elBoundingClientRect) elBoundingClientRect = el.getBoundingClientRect();
		if (isMultiple && !eventsElBoundingClientRect) eventsElBoundingClientRect = eventsEl.getBoundingClientRect();
		...
		if (transformOrigin && params.commonOrigin) $setOrigin(rotateEl, transformOrigin);
		if (highlightEl) {
	...
	var onPointerLeave = function onPointerLeave(e) {
		...
		if (typeof params.rotateTouch === "string" && isScrolling) el.classList.remove("atropos-rotate-touch");
		if (params.alwaysActive) {
			setElements();
	...
	var onDocumentClick = function onDocumentClick(e) {
		...
		if (!eventsEl.contains(clickTarget) && clickTarget !== eventsEl && self.isActive) onPointerLeave();
	};
	var initDOM = function initDOM() {
		...
		if (typeof eventsEl !== "undefined") {
			if (typeof eventsEl === "string") eventsEl = $(document, eventsEl);
		} else eventsEl = el;
		childrenRootEl = isComponent ? el.parentNode.host : el;
		Object.assign(self, { el });
	...
	var init = function init() {
		...
		if (params.shadow) createShadow();
		if (params.highlight) createHighlight();
		if (params.rotateTouch) if (typeof params.rotateTouch === "string") el.classList.add("atropos-rotate-touch-" + params.rotateTouch);
		else el.classList.add("atropos-rotate-touch");
		if ($(childrenRootEl, "[data-atropos-opacity]")) setChildrenOffset({ opacityOnly: true });
...
//#endregion
//#region node_modules/atropos/atropos-react.mjs
/**
* Atropos React 2.0.2
* Touch-friendly 3D parallax hover effects
* https://atroposjs.com
...
function Atropos(props) {
	...
	var init = function init() {
		atroposRef.current = Atropos$1(_extends({ el: elRef.current }, extractParamsKeys(props)));
	};
	...
	return /* @__PURE__ */ import_react.createElement(Component, _extends({ className: cls("atropos", className) }, removeParamsKeys(rest), { ref: elRef }), /* @__PURE__ */ import_react.createElement("span", { className: cls("atropos-scale", scaleClassName) }, /* @__PURE__ */ import_react.createElement("span", { className: cls("atropos-rotate", rotateClassName) }, /* @__PURE__ */ import_react.createElement("span", { className: cls("atropos-inner", innerClassName) }, children, (props.highlight || typeof props.highlight === "undefined") && /* @__PURE__ */ import_react.createElement("span", { className: "atropos-highlight" })), rotateChildren, (props.shadow || typeof props.shadow === "undefined") && /* @__PURE__ */ import_react.createElement("span", { className: "atropos-shadow" })), scaleChildren), rootChildren);
}
//#endregion
